...
window.Engine = {
 ...
 async translatePdf(file,tgt,prog,email){
  ...
  for(let i=0;i<queue.length;i++){
   ...
   const q=queue[i];
   try{
    const g=await gtxTranslate(q,tgt);
    if(i===0&&g.src&&g.src!=='auto')src=String(g.src).split('-')[0].toLowerCase();
    out+=(out?' ':'')+g.text;
   }catch(ge){
    out+=(out?' ':'')+await myMemoryTranslate(q,src,tgt,email);
   }
  }
  return{text:out,src};
 },
...
/* Free translation providers: Google gtx (primary, auto-detect) + MyMemory (fallback) */
async function gtxTranslate(q,tgt){
 const u='https://translate.googleapis.com/translate_a/single?client=gtx&sl=auto&tl='+encodeURIComponent(tgt)+'&dt=t&q='+encodeURIComponent(q);
 const r=await (await fetch(u)).json();
 if(!r||!r[0]||!r[0].map)throw new Error('Google translate failed');
 return{text:r[0].map(x=>x[0]).join(''),src:r[2]||'auto'};
}
async function myMemoryTranslate(q,src,tgt,email){
 let u='https://api.mymemory.translated.net/get?q='+encodeURIComponent(q)+'&langpair='+src+'|'+tgt;
